...
if [ "$NEEDS_BUILD" = "NO" ] && [ -d "$TARGET" ]; then
  ...
  if "$CURL" -fsSL "$RAW_ROOT/$REMOTE_SHA/RECOVERY/SUPRA_AUTOBUILD_SELF_UPDATE_V1.sh" -o "$SELF_REFRESH_TMP" &&
    ...
    SELF_REFRESH_DST="$UPDATER_DST.new.$"
    cp "$SELF_REFRESH_TMP" "$SELF_REFRESH_DST"
    chmod 700 "$SELF_REFRESH_DST"
    mv -f "$SELF_REFRESH_DST" "$UPDATER_DST"
...
if [ -f "$SRC/RECOVERY/SUPRA_AUTOBUILD_SELF_UPDATE_V1.sh" ]; then
  SELF_REFRESH_DST="$UPDATER_DST.new.$"
  cp "$SRC/RECOVERY/SUPRA_AUTOBUILD_SELF_UPDATE_V1.sh" "$SELF_REFRESH_DST"
  chmod 700 "$SELF_REFRESH_DST"
  mv -f "$SELF_REFRESH_DST" "$UPDATER_DST"
fi
